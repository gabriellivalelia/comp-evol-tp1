...
\paragraph{Parâmetros efetivamente utilizados no algoritmo:}
O algoritmo Tabu Search foi implementado com os seguintes parâmetros principais:
\begin{itemize}
    \item \textbf{\texttt{tabu\_tam}}: Tamanho da lista tabu (padrão: 10)
    \item \textbf{\texttt{max\_iter}}: Número máximo de iterações (padrão: 100)
    \item \textbf{\texttt{max\_iter\_sem\_melhoria}}: Critério de parada por iterações sem melhoria (padrão: 30)
    \item \textbf{\texttt{alpha}}: Parâmetro de ponderação da função objetivo multiobjetivo, utilizado como $\alpha = 0.85$ nos experimentos principais (ver Equação~\ref{eq:funcao_objetivo}).
    \item \textbf{\texttt{beta}}: Peso interno para a soma das notas dos bares na função de avaliação incremental do código (padrão: 25.0). Não corresponde ao $\alpha$ da função objetivo final, mas atua como fator de ajuste na busca local.
    \item \textbf{\texttt{usar\_solucao\_inicial\_inteligente}}: Uso de solução inicial baseada em vizinho mais próximo (padrão: True)
    \item \textbf{\texttt{tempo\_visita}}: Tempo de permanência em cada bar (padrão: 1 hora)
    \item \textbf{\texttt{hora\_inicial}}, \textbf{\texttt{hora\_final}}: Janelas de tempo diárias definidas pelo usuário
\end{itemize}
