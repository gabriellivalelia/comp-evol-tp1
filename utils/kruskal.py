...
def visualizar_agm(arestas_agm, df_bares=None):
    ...
    for idx, (i, j, peso) in enumerate(arestas_agm, 1):
        if df_bares is not None:
            ...
            linhas.append(f"{idx:3d}. [{i:3d}] {nome_i:30s} <---> [{j:3d}] {nome_j:30s} : {peso:8.2f}")
        else:
            linhas.append(f"{idx:3d}. Vértice {i:3d} <---> Vértice {j:3d} : Peso {peso:8.2f}")
...
def comparar_com_tsp(custo_agm, custo_tsp):
    ...
    return {
        ...
        'qualidade': 'Excelente' if percentual < 20 else 'Boa' if percentual < 40 else 'Razoável'
    }
